...
if st.button("📄 დოკუმენტების გენერაცია", type="primary", use_container_width=True):
    with st.spinner("მიმდინარეობს გენერაცია..."):
        pick_zip, nveh, npick = docgen.build_picklist_zip(df, date_str, signature=sig)
        del_zip, ndel, ndel_files = docgen.build_delivery_zip(
            df, date_str, signature=sig, group_by_vehicle=veh_folders
        )
    st.session_state["result"] = {
        "token": token, "pick": pick_zip, "del": del_zip,
        "nveh": nveh, "npick": npick, "ndel": ndel, "ndel_files": ndel_files,
    }

res = st.session_state.get("result")
if res and res["token"] == token:
    st.success(
        f"მზადაა: Pick List — {res['npick']} ფაილი "
        f"({res['nveh']} მანქანა × [შესაგროვებელი სია + მიღების ფურცელი])  ·  "
        f"Delivery Notes — {res['ndel_files']} ფაილი "
        f"({res['ndel']} მაღაზიის ფურცელი + {res['nveh']} შესაგროვებელი სია)"
    )
    d1, d2 = st.columns(2)
    d1.download_button(
