...
with st.sidebar:
    ...
    veh_prefix = st.checkbox(
        "მიღების ფაილს დაერქვას მანქანის პრეფიქსით",
        value=True,
        help="ZIP-ში ფაილები დაჯგუფდება მანქანის (მარშრუტის) მიხედვით.",
    )
...
# ---------------------------------------------------------------- generate
token = hashlib.md5(file_bytes).hexdigest() + f"|{date_str}|{sig}|{veh_prefix}"
...
if st.button("📄 დოკუმენტების გენერაცია", type="primary", use_container_width=True):
    with st.spinner("მიმდინარეობს გენერაცია..."):
        ...
        del_zip, ndel = docgen.build_delivery_zip(
            df, date_str, signature=sig, vehicle_prefix=veh_prefix
        )
